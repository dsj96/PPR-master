        ...
        if G.nodes[n]['type'] == 'P':
            ...
            for nbr in nbrs.keys():
                if G.nodes[nbr]['type'] == 'P':
                    ...
                    weight_seq = G[n][nbrs]['weight']
                    out_nbrs_seq_weight[nbr] = weight_seq
            if len(out_nbrs_dist) != 0:
                sum_dist_power = 0.0
                for k,v in out_nbrs_dist.items():
                    sum_dist_power += pow(v, kappa)
                for k,v in out_nbrs_dist.items():
                    out_nbrs_dist[k] = pow(v, kappa) / sum_dist_power
                for k,v in out_nbrs_seq_weight:
...
if __name__ == "__main__":
    ...
    theta = 24 # 
    PATH = str(sys.argv[1])
    u_p_edges, p_p_edges, u_u_edges = load_training_data(PATH+'train_checkin_file.txt', PATH+'friendship_file.txt', istag=False, isweek=False, hour_interval=4, edge_interval = theta)
    history = read_history(PATH+'train_checkin_file.txt')
    poi_longi_lanti = gen_poi_coordinate(PATH+'train_checkin_file.txt')

    G = get_G_from_edges(u_p_edges, p_p_edges, u_u_edges, history, epsilon)
    node_type = gen_node_type_without_tag_from_G(G, PATH+'train_checkin_file.txt', PATH+'friendship_file.txt')
    G = add_geo_type_info(G, poi_longi_lanti, node_type)
    G = update_weight_by_geo(G)

    write_dict_to_file(node_type, PATH+'node_type.txt')
    write_G_to_file(G, PATH+'G_edges.txt')
